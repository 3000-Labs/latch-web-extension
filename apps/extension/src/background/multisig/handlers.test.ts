...
vi.mock('../backend', () => ({
  ...
  multisigProposalApproveWebauthn: (...args: unknown[]) =>
    multisigProposalApproveWebauthn(...args),
...
describe('tryHandleMultisigMessage', () => {
  ...
  describe('error propagation from backend', () => {
    ...
    it('rejects when createMultisigProposal API fails after ensureRegistered', async () => {
      ...
      const sendResponse = vi.fn()
      await expect(
        tryHandleMultisigMessage({ type: 'MULTISIG_CREATE_PROPOSAL', payload: req }, sendResponse, ok)
    ...
    it('rejects when ensureMultisigAccountRegisteredForSession fails before list proposals', async () => {
      ...
      const sendResponse = vi.fn()
      await expect(
        tryHandleMultisigMessage({ type: 'MULTISIG_LIST_PROPOSALS', payload: req }, sendResponse, ok)
    ...
    it('rejects when executeMultisigProposal API fails', async () => {
      ...
      const sendResponse = vi.fn()
      await expect(
        tryHandleMultisigMessage({ type: 'MULTISIG_EXECUTE_PROPOSAL', payload: req }, sendResponse, ok)
